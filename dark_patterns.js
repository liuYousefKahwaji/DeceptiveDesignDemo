...
const PATTERNS = [
  ...
  },
  {
    id: "urgency",
    title: "False Urgency",
    tagline: "Manufactured countdowns and fake activity stats.",
    mechanismName: "False Social Proof & Scarcity",
    mechanismDesc: "Exploits loss aversion and social herd mentality. By flashing countdown timers, showing fake live viewers, and fabricating stock indicators ('only 2 left'), the interface triggers fear of missing out (FOMO) to bypass critical thinking.",
    darkTitle: "Product Detail Checkout",
    darkUrl: "urgency-sales.net/flash-deal",
    cleanTitle: "Product Detail Checkout",
    cleanUrl: "transparent-store.com/deal",
    darkExplain: "The ticking countdown, the view counter, and the stock alert are coded to reset on refresh. They do not reflect real data and serve only to press a quick purchase.",
    cleanExplain: "Pricing policy is static and transparent. Scarcity warnings are only used if they reflect actual system stock. No fake live-view metrics are presented.",
    darkRender: function(containerId) {
      const el = document.getElementById(containerId);
      
      el.innerHTML = `
        <div class="mock-ui-container">
          <div class="urgency-badge" id="timer-badge">
            <span>⏰ Flash Sale Ends In:</span>
            <span id="dark-clock" style="font-family: var(--font-mono); font-weight: bold; margin-left: 4px;">05:00</span>
          </div>
          <div class="urgency-badge" style="animation-delay: 0.2s">🔥 34 users are viewing this deal right now!</div>
          <div class="urgency-badge" style="animation-delay: 0.4s">⚠ Only 2 seats left at this price point!</div>
          <h4 class="mock-title">Yearly Premium Pass</h4>
          <p class="mock-desc" style="font-size: 1.1rem; color: var(--color-danger); font-weight: 700; margin-bottom: 0.5rem;">
            $59.00/year <span style="text-decoration: line-through; font-size: 0.8rem; color: var(--text-muted); font-weight: normal;">$149.00/year</span>
          </p>
          <button class="btn-ui btn-ui-danger" id="btn-buy-urgency">Claim My Discounted Spot Now</button>
          <div id="urgency-dark-alert"></div>
        </div>
      `;
      
      // Start Countdown Timer
      let duration = 300; // 5 mins
      if(window.darkTimerInterval) clearInterval(window.darkTimerInterval);
      window.darkTimerInterval = setInterval(() => {
        const min = Math.floor(duration / 60);
        const sec = duration % 60;
        const clockEl = document.getElementById("dark-clock");
        if(clockEl) {
          clockEl.innerText = `${min.toString().padStart(2, '0')}:${sec.toString().padStart(2, '0')}`;
        }
        duration--;
        if(duration < 0) duration = 300; // loop reset
      }, 1000);
      
      document.getElementById("btn-buy-urgency").addEventListener("click", () => {
        document.getElementById("urgency-dark-alert").innerHTML = `
          <div class="mock-alert danger-alert">
            ⚠ <strong>Purchase logged!</strong> Note: The clock will reset back to 05:00 on page reload, and the stock is infinite. You were pressured by fake metrics.
          </div>
        `;
      });
    },
    cleanRender: function(containerId) {
      ...
      el.innerHTML = `
        <div class="mock-ui-container">
          <div class="urgency-badge-green">✔ Transparent Flat Pricing</div>
          <h4 class="mock-title" style="margin-top: 0.5rem;">Yearly Premium Pass</h4>
          <p class="mock-desc" style="font-size: 1.1rem; color: var(--color-success); font-weight: 700; margin-bottom: 0.5rem;">
            $59.00/year <span style="font-size: 0.85rem; color: var(--text-secondary); font-weight: normal;">- billed annually</span>
          </p>
          <p class="mock-desc">Enjoy full features. 30-day money-back guarantee. No artificial scarcity or ticking clocks.</p>
          <button class="btn-ui btn-ui-success" id="btn-buy-urgency-clean">Subscribe to Premium</button>
          <div id="urgency-clean-alert"></div>
        </div>
      `;
      document.getElementById("btn-buy-urgency-clean").addEventListener("click", () => {
        document.getElementById("urgency-clean-alert").innerHTML = `
          <div class="mock-alert success-alert">
            ✓ Subscription complete. Welcome to Premium! Billed transparently at $59.00/yr.
          </div>
        `;
      });
    }
